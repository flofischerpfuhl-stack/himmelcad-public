...
| WP-G2  | Command-row document, G-1 coverage, generated console/SDK consumption, and Python smoke.                         | **PARTIAL**  | `docs/photolab-automation-command-rows.md`; PhotoLab renderer 87/87 includes both G-1 assertions; [PL-R1](builder-program/evidence/PL-R1-release-inputs-2026-09-19.md) regenerated/reviewed the SDK (manifest-only contract pin), SDK **14/14**, command-table freshness plus app **83/83**, automation host **48 passed/1 pinned-version skip**. **Exact match: no**—the generated clients still expose no public PhotoLab command methods and the real sync/async smoke is absent. | Generate the public PhotoLab SDK rows and compliant brokered-grant smoke harness, then execute G-2 async + sync against the real dataset.      | [R22](#r22-python-automation) (1–3 h after the missing public surface/harness lands; laptop/Windows PC; no owner).               |
...
PL-R1 closed regeneration and the 14/14 SDK gate. The real smoke remains open:
the generated sync/async clients have no public PhotoLab command methods and no
compliant brokered-grant smoke harness exists; the command-row specification
forbids counting a private sidecar RPC substitute.
...
3. **Automation manifest drift completed; R22 remains:** the regenerated SDK is
   current and 14/14. Public PhotoLab sync/async methods and the compliant
   brokered-grant smoke harness must land before the 1–3 h real smoke can run.
