...
| WP-F4  | Exact-candidate Windows inventory and native install/startup if Windows is supported.                            | **PARKED**   | Plan §R1 triage (2026-09-05) parks signing/operator work and explicitly leaves native certification open **if Windows is supported**. WIN-02 opened only a dev shell; WIN-16 is a sidecar workflow, not installer evidence. 2026-09-19 inventory failed closed on missing cross-target sidecar. **Exact match: conditional and not run.**                                                                                  | Owner decides Windows support/signing. If supported: build exact candidate, pass inventory, install NSIS, launch native app and verify signature/update policy. | [R10](#r10-windows-release) (1–3 h; Windows PC unlocked; owner decision + GUI).                                                  |
...
| WP-G2  | Command-row document, G-1 coverage, generated console/SDK consumption, and Python smoke.                         | **PARTIAL**  | `docs/photolab-automation-command-rows.md`; PhotoLab renderer 87/87 includes both G-1 assertions; command-table check passed; app 80/80; automation host 48 passed/1 version-pin skip. Python SDK was **13/14**: stale `generator-manifest.json`. **Exact match: no**—generated tree is stale and real Python import→align→optimize→product smoke absent.                                                                  | Regenerate/review SDK, rerun its 14 tests green, then execute G-2 async + sync client smoke against the real dataset.                                           | [R22](#r22-python-automation) (1–3 h; laptop/Windows PC because alignment is heavy; no owner).                                   |
...
All commands below ran once on 2026-09-19 unless an environmental retry is
explicitly stated. Rust commands used
`CARGO_TARGET_DIR=/home/oem/Dokumente/003_Projekte/10_himmelcad/target/photolab`
...
- Generated surfaces: command-table staleness check passed; app **80/80**;
  automation host **48 passed, 0 failed, 1 skipped** (real Codex probe version
  pin expected 0.144.5, installed 0.155.1).
- Python SDK: **13 passed, 1 error**; generated-tree check reports stale
  `sdk/python/generator-manifest.json`. This is a product/repository failure,
  not an environmental retry.
...
- Release inventory: Linux failed closed because
  `.build/photolab-runtime/linux-x64/workers/colmap` is missing; Windows failed
  closed because
  `target/x86_64-pc-windows-gnullvm/release/himmelcad-sidecar.exe` is missing.
...
Regenerate and review the Python SDK, require all 14 SDK tests, then execute
the G-2 async and sync import→align→optimize→product→reopen smoke defined in
`docs/photolab-automation-command-rows.md`. Estimate 1–3 h due to alignment.
...
2. **Restore deterministic release inputs:** stage the Linux COLMAP subtree and
   exact Windows cross-target binaries; rerun both release inventories. Laptop
   10–15 min, Windows PC 30–60 min; no human unless staging fails.
3. **Fix automation drift:** regenerate/review the Python SDK manifest, reach
   14/14, then run R22. The real smoke takes 1–3 h on laptop/PC; no owner.
