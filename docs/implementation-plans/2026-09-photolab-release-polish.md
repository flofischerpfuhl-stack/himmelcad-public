...
Stage 1 code landed 2026-09-04 (landing pending the dense-mesh smoke and the light-theme G17 review): mesh configuration gains `meshSource: dem | dense` (default `dem`, DEM path byte-identical); `dense` takes the frozen published dense cloud and runs `colmap poisson_mesher --input_path <fused.ply> --output_path <mesh.ply> --PoissonMeshing.depth 11 --PoissonMeshing.trim 7` (X6 tunables) under the existing supervised-child cancellation (kill within the bound, candidate discarded, dense input survives), then the existing PLY tiler publishes it with the 5-tuple lineage. Provenance: the core descriptor and the durable job identity freeze source, dense run id and fused-cloud SHA-256; the review added `MeshProvenance` to `MeshArtifactRecord` (`meshSource`, `denseRunId`/`sourceDemEntityId`, `sourceArtifactSha256`; older records carry none — all were DEM drapes). Known stage-1 limits (Codex gap 1, accepted): the PLY tiler has neither a face-count decimation input nor a vertex-colour channel, so the dense mesh is untextured geometry and the face-count field is hidden for `dense`; the panel says "Stage 1 produces true 3D geometry; image texturing follows in a later release." e2e gained `--mesh-source dem|dense`. Verified: sidecar 273 lib + 13 + 92 bin (fake `poisson_mesher` rig: args, cancellation, tiler hand-off), core 217, renderer 86, typecheck, English UI, prettier. G17 dark review accepted (`.build/photolab-ui/a3-dark/function-textured-mesh{,-dense}.png`). Light review accepted too (`a3-light`). Smoke findings 2026-09-04 (24 images, `--products depth,dense,mesh --mesh-source dense`): (1) the Poisson mesh had 1,930,836 of 2,419,096 faces with exactly zero area because COLMAP meshes in float32 and the fused cloud sits at projected coordinates (~4.4e6 m easting) — fixed by meshing in a local float32 frame (`dense_raster_prep::write_dense_local_frame_ply`, origin = per-axis floor of the minimum, colours + normals kept) and restoring world coordinates in the generated-mesh adapter (601efde); remaining zero-area faces are dropped and counted (`degenerateFacesDropped` on the mesh provenance, 1148ffd); (2) mesh packages could never publish: the prepared-mesh dataset used representation slot `primary` while its admission uses `source` (latent since WP-G1a; fixed in 601efde). Confirming smoke 2026-09-04 18:5x (24 images, `--products depth,dense,mesh --mesh-source dense`, clean-worktree build of ea66991): green — the Poisson mesh publishes as a prepared hierarchy with 4,186,807 triangles in 99 tiles (596 tile files, 560 MB), open surface (not a closed manifold → `hcad.surface-3d@1`), provenance `{meshSource: dense, denseRunId, sourceArtifactSha256, degenerateFacesDropped: 2582}` — down from 1,930,836 zero-area faces before the local-frame fix. Stage-1 acceptance met (publish + prepared for the viewer + lineage; PLY export is the existing product-export path; cancellation covered by the fake-mesher test). **WP-A3 stage 1 landed.** Evidence under `.build/photolab-evidence/a3/`. Incident during the landing: commit 601efde had staged five Builder-lane WIP hunks of main.rs; removed from HEAD in ea66991 (working tree untouched); staging rule tightened in the review protocol.
...
- Evidence: open — not executed
- Status: landed 2026-09-08 (b529641; types in 0fc86cb) — gate evidence pending
```

Landed 2026-09-08 (b529641, Codex gpt-5.6-sol high, 544 k tokens; reviewed by
the PhotoLab session): per-machine budgets at admission (available − max(4 GiB,
12.5 %) − running jobs), measured models (extraction 750 B/actual pixel/worker;
LightGlue k² × 4 B × 3 layers + 256 MiB ≈ 7.2 GB at 24k; SIFT 256 MiB), time
first (workers, sequential pair batches), quality only per the unit-of-work
rule (`extractionEdgeReduced`, `matchingKeypointsCapped`, typed in job record,
lineage, Jobs/report status), peak-RSS sampling per stage by the process-group
supervisor, MVS stage-boundary peaks, memory preflight with
`InsufficientMemory`. Reviewer changes: the fixed one-worker neural extraction
cap Codex proposed was removed (owner S23 — the model alone gives 1 worker at
27 GiB usable, 2 at 32 GiB, 3 at 56 GiB; test rewritten with those cases); the
Jobs status line was shortened and made wrapping after the G17 dark shot showed
it truncated ("Memory envelope: keypoints capped 24,000 to 18,000 · 1
extraction worker · 2 matching workers"); dark + light accepted. Verification:
core 229 + 1, sidecar lib 302 (+3; the two Builder-lane failures unchanged;
three cancellation-timing tests failed once under load 12 and passed serially),
bins 96, MVS 13, renderer 84, typecheck clean; the exact staged subset (HEAD +
A7-only `main.rs` hunks, derived by reverse-applying the Builder hunks) was
cargo-checked with all targets in a scratch worktree. Open: the tunables rows
are in the register; gate 2 evidence (135-image Quality Hybrid on this laptop
within the envelope + 16 GB-limited rerun) and the report rendering of the
...
### WP-A7e — Recalibrate neural matching and enforce resident limits (Size S)

Implemented 2026-09-09 (working tree, no commit; smoke pending): the LightGlue
model is recalibrated to 12 attention layers from the two 8,122-keypoint OOM
incidents (26.4 GB and 28.8 GB anon RSS for eight threads; model 27.5 GB). On
the 29.1 GB usable reference machine, the 50% matching share therefore plans
four workers at the fast-profile 8,192-keypoint cap. Linux workers use a
process-cached `systemd-run --user --scope` availability probe and a transient
scope with `MemoryMax`, no swap and collection; the fallback keeps child-only
`RLIMIT_AS` at twice the resident limit because the A7d smoke reached only
6.31 GB RSS at its 10.6 GB virtual-address ceiling. Stage parameters record
`modelBytes` and `workerMemoryLimitMode` (`cgroupScope` or `rlimitAs`) beside
the measured peak. Ready for the A7d smoke rerun with unit `MemoryMax=18G`;
...
Landed 2026-09-04 (scoped to admission + disk preflight; the side-channel part shipped with WP-H2): `JobManager` admission carries frozen publication targets (kind key, target entity, lineage target; batches carry the union) and rejects a queued or running collision with `ConflictingTarget` (RPC code `conflictingTarget`, message "A <kind> for this <target> is already <queued|running> (job <id>). Wait for it or cancel it."); terminal jobs never conflict. Disk preflight estimates scratch + output bytes per kind (X6 table: alignment 2 GiB + 8 MiB/image, depth 40 MiB/image, dense 1.5× depth, DEM/ortho 4× a four-byte pyramid incl. the 4/3 overview factor, mesh 2 GiB, splat 6 GiB — raster steps of a batch that creates its own alignment contribute no speculative extent) and rejects with `InsufficientDisk` ("Not enough free space on <volume>: about <required> needed, <available> free."); free space via `df -k --output=avail` / `fsutil volume diskfree`, queried before scheduler/history admission. The renderer shows both inline beside Start (existing error box; helper `inlineProductStartError`). Gaps accepted as read: the target types live sidecar-side (`JobAdmission.publication_targets`, a vector for batch unions) rather than on core `NewPhotolabJob`; the conflict variant carries the frozen admission state to word the message. G17: no new visual surface (existing inline error box); the copy was reviewed textually because the harness mock cannot reach Start (no published alignment) — a capture step is queued for when the mock carries one. Verified: sidecar 271 lib + 13 + 90 bin (job_runtime 30, project_runtime 61), renderer 83 + helper test, typecheck, English UI, prettier.
...
2. **Gates** — a test that enumerates ribbon/panel actions against the row
   list (every UI-reachable operation has a row), plus the PL-I2 takeover's
   bounded brokered-grant Python smoke (project create → image inspect/commit
   → jobs list → cancellable job start/cancel → close) through the sync and
   async clients. Alignment/COLMAP/ALIKED/MVS work is outside this R22 smoke.
...
| WP-A7   | OS/UI reserve = max(4 GiB, 12.5 % of physical RAM)                                                                                                                         | Keeps the desktop responsive on 16–64 GB machines; measured: a 32 GB laptop with 4 GB reserved never swapped during the fast smokes                              |
| WP-A7   | Extraction memory model 750 B per actual resized pixel per neural worker (calibration constant, re-measured per run)                                                       | Measured 15.7 GB RSS for one ALIKED_N32 worker on a 21 MP frame; recalibrated from each run's recorded peak                                                      |
| WP-A7   | Neural matching model k² × 4 B × 12 attention layers + 256 MiB base per worker; SIFT 256 MiB per worker                                                                    | Calibrated from two 8,122-keypoint, eight-thread OOM incidents: model 27.5 GB total versus 26.4 GB and 28.8 GB anon RSS; SIFT measured below 256 MiB on Sulzberg |
| WP-A7   | Matching stage share 50 % of usable memory; extraction uses the remaining usable                                                                                           | Matching is the stage that killed the machine; half the envelope leaves room for the extractor's file cache and the UI                                           |
| WP-A7   | Quanta: extraction edge multiples of 256 px; keypoint cap multiples of 500; neural extraction workers bounded only by the model and logical CPUs (no fixed cap, owner S23) | Coarse quanta keep degradation records readable and reproducible; the model alone yields 1 worker at 32 GB and 3 at 64 GB                                        |
...
F15 (2026-09-05, Builder-lane a540935): the shared EntityTree's context menu became the command-table `EntityCommandMenu`, dropping PhotoLab's `remove` and `showGcpImages` actions ("Remove from project…" with its confirmation dialog; "Images containing this GCP"). Disposition: **fix before release** via the command table (P11: rows `photolab.images.remove`, `photolab.gcp.images` with surface contextMenu; EntityTree forwards `photolab.*` invocations) — Builder lane owns the extension point, PhotoLab adapts `handleTreeContextAction`. Fixed 2026-09-06: Builder S-06c (5c78e69) added the product rows `photolab.images.remove` ("Remove from project…", kind CameraImage, multi-select) and `photolab.gcp.images` ("Images containing this GCP", kind GroundControlPoint) to the generated command table with surface contextMenu and a product filter; the shared EntityTree forwards them as `(commandId, entityIds)` when `productId` is set. PhotoLab passes `productId="photolab"` and maps both ids onto its existing remove/showGcpImages flows (confirmation dialog unchanged); the two ids are P11 rows in `automationCommandRows.ts` and the row table (G-1 green). Baselines and the gate-7 audit are regenerated once the Builder's GPU baseline finishes.
...
Landed 2026-09-04 (adapter, no PhotoLab-private registry): the five project-runtime side-operation registries (archives, image inspections, image commits, image masks, GCP operations) are cancelled and awaited by `drain_side_operations`, refuse admission while draining, and appear in `photolab.jobs.list` as synthetic `PhotolabJob`s with `origin: sideOperation` (kinds archiveSave/imageInspection/imageCommit/imageMask/gcpOperation; `photolab.jobs.status`/`cancel` route to the owner; no durable terminal history by design). Renderer: status-bar jobs chip from the pure helper `jobsChip.ts` (hidden / `n jobs running` / `1 job running · label pct` / `Cancelling…` / `Job failed — label` until Jobs is opened / 4 s completion linger, X6), tone through tokens, opens the Jobs tab and restores the previous tab when toggled closed; the first poll is immediate so a reload rehydrates from the sidecar truth. Toast delivery stays pending the Builder-lane primitive (S-05). Obligation recorded: Builder's S-05 builds the shared status-bar chip + jobs island in `@himmelcad/ui`; when it lands PhotoLab adopts it and deletes the local chip (one component, not two). Fulfilled 2026-09-05 (WP-H2b, 6cc334b, pushed): PhotoLab renders the shared `JobsStatusChip` (S-05, a540935) through the pure mapping `jobSurfaceItems` (queued → pending-registration; running/paused/pauseRequested → running with paused/pausing phases; cancelRequested → cancelling; completed/failed/cancelled; `fraction` null when units are unknown; side operations cancel at the next safe boundary); the local chip and its CSS are deleted; the Jobs-tab toggle and the failed-job acknowledgement are unchanged. G17 review 2026-09-05 10:05 (own eyes, dark + light `00-main-view`/`bottom-jobs` from a private worktree of HEAD): accepted — the pill, its progress slot and tones read as the same family as Builder's gallery; the light-theme viewport toolbar (F13) is fixed by the shared components. Verified: renderer 84 tests, lint clean; workspace typecheck re-run when the Builder lane's V-02 lands; pixel baselines follow once F15 restores the tree context menu (the harness stops at that step). G17 review 2026-09-04 13:40 (own eyes, dark + light `00-main-view`/`bottom-jobs` with one running job and one running side operation): accepted. Polish queued as F14: a side operation without known units shows `overall 0%`; `in progress` would be more honest (existing convention for all jobs). Verified: sidecar 265 lib + 13 + 86 bin, core 216, renderer 83 (jobsChip 6), prettier; the workspace typecheck was blocked by Builder-lane S-05 WIP at landing time and is re-run when it is consistent.
...
Shared-file staging rule (after incident 601efde, 2026-09-04): in the shared working tree `git add <file>` also stages the other lane's uncommitted hunks. For shared files (sidecar `main.rs`/`lib.rs`, `packages/@himmelcad/*`, shared core modules) stage hunk-by-hunk (patch of own hunks → `git apply --cached`) and grep the staged diff for foreign identifiers before committing; repair a slip with an index-only reverse patch so the peer's working tree stays untouched.
