...
- **WP-H1** (Size S, gates 1/3): F01 archive Save route + F05 close refusal
  with Retry / Cancel close / Force quit.
- **WP-H2** (Size M, gates 3/4): side-operation drain coverage (inspection,
  mask, GCP operations) + global jobs chip rehydrating from the sidecar job
  list after reload (adapter, not a registry).
- **WP-H3** (Size M, gate 7): Escape dispatcher with rungs, rename
  Escape/blur fix (shared, announced), function close action, cancelling
  state, selection preservation/revalidation.
- **WP-H4** (docs): per-package A1–E3 footer and evidence ledger.
- **WP-H5** (evidence): executed R1 gate ledger for the release candidate.
