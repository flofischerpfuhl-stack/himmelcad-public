...
**`G-VC-TRANSITION`: FAIL for the captured run.** The idle Class-I baseline measured 3D→2D→3D at p50 `156.0 ms`, p95 `206.0 ms`, and p99/max `206.0 ms`, versus the `33.4 ms` motion target. Measurement state: 09:20:31 CEST uptime load `0.56 / 1.13 / 3.09`, 21 GiB free / 28 GiB available; Quadro M2200 driver `580.173.02`; hardware WebGL2 via ANGLE/Vulkan; DPR approximately 1 at 1440×900; present source `raf-render-complete`. The result is lower than the earlier contended 237.5 ms p95 but still fails; no tuning was performed. The landed launcher records one transition repetition rather than the specified five-run median, so it cannot establish a protocol-complete pass.
