...
## Windows host (owner decision 2026-09-08)

The Windows PC DESKTOP-BNB2PBA (Tailscale 100.100.224.9, user `flo`, OpenSSH key-only, Codex CLI authenticated there) is the project's Windows build/test/measurement host. Rule: the architect does not run commands there; it prompts the Windows Codex with briefs via `.claude/codex/run-remote.sh` (logs `.claude/codex/out/remote-<name>.*`), and results come back as committed evidence. Consequence: all Windows-specific build artifacts on the Linux laptop (wine prefixes, win32 runtime staging, COLMAP Windows build, vcpkg Windows) are deleted; Windows verification, the PhotoLab golden run and GPU baselines of class W/D move to the Windows host.
