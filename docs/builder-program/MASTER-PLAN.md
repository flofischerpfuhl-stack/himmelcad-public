...
- **V-01c (2026-09-08):** Electron reaches the Quadro M2200 only via ANGLE/Vulkan WebGL2 (`HIMMELCAD_GPU=nvidia` opt-in; harness default); the WebGPU present path fails on NVIDIA 580.173 + Electron 43 (`VK_ERROR_FEATURE_NOT_PRESENT`, external-memory import), direct PRIME EGL fails. The Quadro run under load 12–17 measured p95 360–680 ms — adapter evidence only. Disposition: Class W/D qualification (VC §1.1) moves to the Windows host (V-01d after WIN-00 bootstrap: discrete GPU, WebGPU/DX12 via Electron on Windows, quiet machine); the laptop remains Class-I diagnostic. The NVIDIA WebGPU present failure is filed as a kernel/driver compatibility finding for V-07 qualification, not fixed here.

- **WebGL2 backend gate (2026-09-08 22:10, from PhotoLab's harness):** V-05's EDL depth-texture read (`textureLoad` on a depth texture) is unsupported on the GLSL/WebGL2 backend and blanked the viewport in the shared tree. Standing gate for every render-crate slice from now on: the PhotoLab visual harness (Chrome headless, WebGL2) captures a non-blank viewport with zero GPU-device errors before the slice is committed; effects that need WebGPU-only features are tiered off on the GL backend. Applied to V-05 at landing (V-05b if needed).

- **All Release-0.5 slices landed (2026-09-09 03:20):** 0.5-01…0.5-08 incl. 0.5-02a segmentation, plus V-01…V-06 of the Viewer Core, on origin/main ba54e0c. Open, bundled in **Q-01 qualification** (idle machine, after the PhotoLab smokes): viewer baseline class I, VB-D7/VB-D8, G-RW-SEGMENT, draw snap latency, G-VC-TRANSITION, LOD continuity, G-MT-3/5, mesh recovery kill/restart, Brandenburg (fixture missing). Next after Q-01: **R-01 demanding-user workflow reviews** of the 0.5 spine (import → view → box → ground → sample → lines → DGM create → edit → export → measure) on the real road scan through visible UI only, **R-02 fixes** from those reviews, **R-03 alpha packaging** (no installer; Linux + Windows builds via the Windows host), then the "usable at starter level" acceptance with the owner and pilot offices. Repo-policy fix: the Rust-generated data bindings were locally excluded since 2026-08-09 and never on origin — now tracked (284dcf6).

- **R-01 verdict (2026-09-09 17:00): Release 0.5 is NOT usable at starter level.** Through visible UI the reviewer could not import the road LAS (File ▸ Import offered only PhotoLab datasets — a G1b regression), New blanked the renderer until restart, and the first boundary failed with a canonical `default-layer` duplicate/tombstone error. R-02 (P0 fixes: import reachability, New/project replacement, default-layer lifecycle; bounded P2 chrome/measurement items) runs now; then R-01 is rerun twice (scan-first, boundary-first). P1 packages queued: viewing-box contract alignment, point-cloud prerequisites/hand-off, DGM loop qualification on the road scan, Q-02 viewer gates (V-07b, VB-D7/8, LOD/EDL, segmentation parity). PhotoLab lane offline since 11:55 → architect runs G1a-3b (validity masks) and the Windows WIN-01/WIN-02 runs; A7c next if still offline.

- **R-03 prerequisite (found by R-02, 2026-09-09 18:50):** `pnpm --filter @himmelcad/builder package:linux` stops at the staging integrity check because the local automation runtime manifest is stale — the `runtime/automation-runtime-manifest.json` on this machine (Jul 20) carries `automationSchemaSha256 cbc5fc44…` while the automation schema is now `e770e37f…` (R-02 added import rows). Packaging needs the automation runtime release rebuilt against the current schema (Builder-lane chore, `scripts/build-automation-runtime-release*` / `stage-automation-runtime.mjs`) before any alpha package; the dev-run evidence of R-02 stands.

- **R-01b verdict (2026-09-09 22:15): still FAIL.** Import route fixed (the road LAS publishes as a 103.7 M-point entity through the UI); but New blanks the renderer again in the reviewer's flow, the published road is invisible and unselectable (HUD ~0.1 M points), a bounded Viewing Box lock stays at 0 %, and `default-layer` still errors on persisted projects; typed entry commits partial vertices. Neither spine reaches ground extraction. Process change: implementer definition of done now requires the literal-UI flow on the named real dataset (`_impl_brief.md`). R-02b runs on the four P0 items with the reviewer's reproductions as acceptance; R-01c reruns after it.

- **R-02b landed (2026-09-10 01:35, 0c9ef0b):** the four P0 blockers of R-01b fixed and replayed through the literal UI on the road scan (New keeps the shell alive, the road renders and selects, atomic typed entry, default-layer repair, Viewing Box lock 20 s / cancel 269 ms). Spine now stops at Extract Ground (capture 0 %, cancel 225 s) and at Viewing Box → From selection for a boundary → R-02c; then R-01c (third review). Windows: WIN-05 rerun with the disk-probe fix (PhotoLab 6fbdcca) running.

- **Milestone consistency (owner, 2026-09-11):** BIM and Plan/specification management are not part of Release 1.0 (D-07 "specifications and BIM are excluded", D-08 deferred). The M-RW member row "Start of specification management (D-07)" contradicted that exclusion and is removed; Release 1.0 = S16 RealWorks-starter outcome + Gaussian-splat display, with installer/updater/public closure per the deferral table.

- **R-02d landed (2026-09-11 17:24):** the five P0 items of R-01c fixed and the road-scan spine replayed end to end through the literal UI for the first time — import → box lock → ground → fence segment (Keep inside) → sample/raster → boundary/breakline (atomic) → checked DGM (7,466 triangles) created, reopened, smooth-edited, undone → DXF export + re-import. Stops left: LandXML needs an explicit units control in the export island; idle GPU work is unbounded after publication (~422 % GPU-process CPU at rest, no backlog); a segmentation progress flood interrupted the strict one-process New gate. → R-02e (units control, rest-budget governor, progress coalescing), then R-01d (fourth review) after the Codex reset.

- **R-02e landed (2026-09-11 19:47):** LandXML units selector (project default, preflight, metre/foot/US-foot round trip, typed refusal when unset); settled viewer sessions stop presenting (0 frames in the 2 s V-01 sample, 0 % GPU-process CPU at rest); job progress limited to 10 Hz with immediate phase changes; the strict one-process New → import → draw → save → reopen gate passes on the 103.7 M-point road. With R-02d the 0.5 spine now runs end to end through the UI incl. LandXML in the implementer's replay. Next: R-01d (fourth demanding-user review) after the Codex reset, then the P1 list and Q-02.

- **Owner course correction (2026-09-19):** primary goal = PhotoLab completely finished; Builder = testable only (fix crashes/unreachable entries of already implemented functions; no feature growth, no new functions, no further agent review rounds). The owner tests Builder himself and corrects domain behaviour (first corrections, DGM creation: any selection of lines/points with heights can be meshed — points are optional; roles are assigned in the surface tool, never at draw time; thinning is a separate tool and does not belong in the DGM window; the RIB rule "breakline vertex off the point set" is not an error; the surface is an ordinary project object, export is the general export). R-02f's DGM logic edits were reverted; its testability parts (test-only dialog responder, import panel state, close lifecycle) are finished by R-02g.
