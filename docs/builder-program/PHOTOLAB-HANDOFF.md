...
| 2 real-dataset accuracy                | owner 2026-09-08: the Windows PC (DESKTOP-BNB2PBA, Tailscale, SSH key-only; Builder prompts Codex there via `.claude/codex/run-remote.sh`) is the golden host — the 135-image Quality Hybrid golden and any Windows gate evidence run there via Codex briefs once the Builder shares the inventory (GPU/RAM). Local: 24-image fast smokes only (this laptop: CPU-only COLMAP, OOM at Quality Hybrid) |
