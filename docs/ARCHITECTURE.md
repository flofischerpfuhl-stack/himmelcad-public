...
| -------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Model                | Object types, properties, states, validation, typed artifacts. Object data only — view appearance and residency live elsewhere.                                                          | Know files, GPU, UI, products.                                         | `core::{hash, entity*, property_schema, canonical_resource*, typed_artifact, geometry_representation_registry}`, schema half of `release_05_admissions`                             |
| Document             | Project store, commands with expected revisions, journal, undo/redo, generic archive, durable/atomic files, format migrations. The kernel holds a handle; the implementation lives here. | Contain domain algorithms.                                             | `core::{canonical_document, entity_commands, canonical_json}`, `sidecar::{canonical_project_store, durable_fs, publish_fs}`, generic half of `project_archive`                      |
...
| Prepared artifacts   | Renderer-neutral prepared hierarchies and tiles (point, mesh, raster, splat), their manifests and atomic publishers.                                                                     | Depend on the renderer or on domain policy.                            | `sidecar::{dense_raster_prep, mesh_tiler, prepared_triangle_mesh*, viewer_raster*_manifest, splat_tiler}`, `core::product_import_package`, prepared types now in `himmelcad-render` |
| Spatial              | Spatial indexes and queries.                                                                                                                                                             | —                                                                      | `himmelcad-spatial`                                                                                                                                                                 |
| Commands + jobs      | Command registry, protocol dispatch from registrations, generic job supervision (progress, cancel, admission, concurrency), worker processes.                                            | Hold product-specific branches or domain job policy.                   | `core::app_protocol`, `sidecar::{main, automation_runtime, process_group, worker_toolchain}`, generic halves of `job_runtime` and `core::photolab_jobs`                             |
| Domain modules       | One field each (registration, point cloud, surface/DGM, drafting, raster, photogrammetry). Register commands, jobs, panels, tools; domain job policy and domain storage live here.       | Depend on another domain module or a product; bypass the command gate. | spread over `sidecar` (incl. `canonical_app_runtime`, `project_runtime` = PhotoLab storage), `core::{photolab_*, mesh_surface, registration}`, app `App.tsx` files                  |
