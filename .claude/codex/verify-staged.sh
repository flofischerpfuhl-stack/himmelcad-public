...
git checkout-index -a --prefix="$OUT/" -- apps packages crates scripts schemas sdk package.json pnpm-workspace.yaml pnpm-lock.yaml tsconfig.json tsconfig.base.json Cargo.toml Cargo.lock eslint.config.js .prettierrc* .prettierignore 2>/dev/null
...
echo "== root typecheck on the staged index"; (cd "$OUT" && pnpm typecheck >"$OUT/typecheck.log" 2>&1); TS=$?; echo "typecheck exit=$TS"; [ $TS -ne 0 ] && grep -E "error TS" "$OUT/typecheck.log" | head -5
