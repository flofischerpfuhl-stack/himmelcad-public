...
#[cfg(test)]
use crate::ProgressEventCoalescer;
#[cfg(test)]
use canonical_app::handle_app_negotiation;
#[cfg(test)]
use io::{handle_io_formats_page, handle_io_rpc};
...
pub(crate) type SidecarCommandRegistry = CommandRegistry<(), RpcRequest, RpcResponse>;

#[derive(Clone, Copy, Debug)]
pub(crate) struct ProductSelection {
    pub(crate) builder: bool,
    pub(crate) photolab: bool,
}
...
#[derive(Clone)]
pub(crate) struct RouteServices {
    pub(crate) projects: Arc<ProjectRuntime>,
    ...
    pub(crate) canonical_app: Arc<Mutex<CanonicalAppRuntime>>,
}
...
impl RouteServices {
    pub(crate) fn new() -> Result<Self> {
        ...
        Ok(Self {
            ...
            las_imports: Arc::new(LasImportOperations::default()),
            ground_operations: Arc::new(GroundOperations::default()),
...
pub(crate) fn build_command_registry(
    selection: ProductSelection,
    services: &RouteServices,
) -> Result<SidecarCommandRegistry> {
    ...
    .register(&mut registry)?;
    root::RootModule::shared().register(&mut registry)?;

    if selection.builder {
        root::RootModule::builder(
            Arc::clone(&services.las_imports),
            Arc::clone(&services.canonical_app),
        )
        .register(&mut registry)?;
        canonical_app::CanonicalAppModule::builder(
            Arc::clone(&services.canonical_app),
            Arc::clone(&services.automation),
        )
        .register(&mut registry)?;
        ...
        mesh_surface::MeshSurfaceModule::new(
            Arc::clone(&services.ground_operations),
            Arc::clone(&services.canonical_app),
        )
        .register(&mut registry)?;
    ...
    if selection.photolab {
        root::RootModule::photolab(Arc::clone(&services.projects), Arc::clone(&services.jobs))
            .register(&mut registry)?;
        photolab_capture::PhotolabCaptureModule::new(Arc::clone(&services.projects))
            .register(&mut registry)?;
        photolab_crs::PhotolabCrsModule::new(Arc::clone(&services.crs)).register(&mut registry)?;
        photolab_gcp::PhotolabGcpModule::new(
            Arc::clone(&services.projects),
            Arc::clone(&services.crs),
        )
        .register(&mut registry)?;
        photolab_himmelcap::PhotolabHimmelcapModule::new(Arc::clone(&services.projects))
            .register(&mut registry)?;
        ...
        .register(&mut registry)?;
        photolab_products::PhotolabProductsModule::new(Arc::clone(&services.projects))
            .register(&mut registry)?;
...
fn route_definitions(selection: ProductSelection) -> Vec<RouteDefinition> {
    ...
    routes.extend(definitions(root::SHARED_METHODS, "root", "shared"));
    if selection.builder {
        routes.extend(definitions(root::BUILDER_METHODS, "root", "builder"));
        routes.extend(definitions(
            canonical_app::BUILDER_METHODS,
            "canonical-app",
            "builder",
        ));
        routes.extend(definitions(
    ...
    if selection.photolab {
        routes.extend(definitions(root::PHOTOLAB_METHODS, "root", "photolab"));
        routes.extend(definitions(
...
fn routes_to_canonical_app(method: &str) -> bool {
    canonical_app::SHARED_METHODS.contains(&method)
        || canonical_app::BUILDER_METHODS.contains(&method)
}

pub(crate) fn print_registered_routes() -> Result<()> {
    let selection = ProductSelection::ALL;
    let services = RouteServices::new()?;
    let registry = build_command_registry(selection, &services)?;
    let routes = route_definitions(selection);
    anyhow::ensure!(
        registry
            .methods()
            .eq(routes.iter().map(|route| route.method)),
