...
use himmelcad_command::automation_runtime::{AutomationDocument, AutomationObjectSource};
...
use himmelcad_model::entity_model::{
    built_in_type, CanonicalEntity, ElevationSurfaceGeometry, EntityTypeId, GeometryObject,
    RepresentationRole, TriangleMeshStorage, Vector3,
};
...
use himmelcad_document::domain_commands::{
    DomainPreparedArtifact, DomainPreparedDataset, DomainResidencyEntry, DraftingDocumentCommands,
...
impl AutomationDocument for CanonicalAppRuntime {
    ...
    fn automation_entities(&self) -> Result<(u64, Vec<CanonicalEntity>), CanonicalAppRuntimeError> {
    ...
    fn automation_validate_transaction(
        ...
        CanonicalAppRuntime::automation_validate_transaction(self, transaction)
    }
    ...
    fn automation_object_source(
        ...
        CanonicalAppRuntime::automation_object_source(self, object_hash)
    }
}

impl DraftingDocumentCommands for CanonicalAppRuntime {
    type Error = CanonicalAppRuntimeError;

    fn drafting_entities(&self) -> Result<Vec<CanonicalEntity>, Self::Error> {
        Ok(self
            .store
            .as_ref()
            .ok_or(CanonicalAppRuntimeError::ProjectNotOpen)?
    ...
    fn drafting_tombstone_exists(&self, entity_id: &EntityId) -> Result<bool, Self::Error> {
        Ok(self
            .store
            .as_ref()
            .ok_or(CanonicalAppRuntimeError::ProjectNotOpen)?
    ...
    fn drafting_read_object(&self, object_hash: &ObjectHash) -> Result<Vec<u8>, Self::Error> {
        Ok(self
            .store
            .as_ref()
            .ok_or(CanonicalAppRuntimeError::ProjectNotOpen)?
    ...
    ) -> Result<ObjectHash, Self::Error> {
        Ok(self.store_mut()?.put_geometry_object(geometry)?)
    }
