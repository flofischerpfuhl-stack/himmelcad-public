...
/// Domain-owned validation called after bounded extraction and before publication.
pub trait ArchiveManifestValidator {
    fn validate(&self, manifest_bytes: &[u8]) -> Result<(), ProjectArchiveError>;
}

/// Structural validator for the shared canonical project manifest.
#[derive(Debug, Clone, Copy, Default)]
pub struct CanonicalOrVersionedManifestValidator;

impl ArchiveManifestValidator for CanonicalOrVersionedManifestValidator {
    fn validate(&self, manifest_bytes: &[u8]) -> Result<(), ProjectArchiveError> {
        ...
        {
            return validate_canonical_manifest(&value);
        }
        let found = value
            .get("formatVersion")
            .and_then(serde_json::Value::as_u64)
            .and_then(|value| u32::try_from(value).ok())
            .ok_or(ProjectArchiveError::InvalidCanonicalManifest)?;
        if found != 1 {
...
where
    P: FnMut(ArchiveProgress),
{
    unpack_hcadx_with_validator(
        archive_path,
        destination,
        limits,
        cancellation,
        progress,
        &CanonicalOrVersionedManifestValidator,
    )
}

pub fn unpack_hcadx_with_validator<P>(
    archive_path: &Path,
    destination: &Path,
    limits: UnpackArchiveLimits,
    cancellation: &CancellationToken,
    progress: P,
    validator: &impl ArchiveManifestValidator,
) -> Result<ArchiveSummary, ProjectArchiveError>
where
    P: FnMut(ArchiveProgress),
{
    unpack_hcadx_with_cancel_and_validator(
        archive_path,
        destination,
        limits,
        || cancellation.is_cancel_requested(),
...
#[allow(clippy::too_many_lines)]
pub fn unpack_hcadx_with_cancel_and_validator<C, P>(
    archive_path: &Path,
    destination: &Path,
    limits: UnpackArchiveLimits,
    mut is_cancelled: C,
    ...
    P: FnMut(ArchiveProgress),
{
    ...
    validate_manifest(&staging, validator)?;
    check_cancelled(&mut is_cancelled)?;
    emit(
        &mut progress,
        ArchivePhase::Committing,
...
fn validate_canonical_manifest(value: &serde_json::Value) -> Result<(), ProjectArchiveError> {
    ...
    Err(ProjectArchiveError::InvalidCanonicalManifest)
}
