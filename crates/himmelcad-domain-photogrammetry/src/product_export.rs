...
use himmelcad_document::publish_fs::publish_replace;
...
pub struct PointCloudTranscodeSummary {
    pub bytes: u64,
}

pub trait PointCloudTranscoder: std::fmt::Debug + Send + Sync {
...
#[derive(Debug)]
struct UnavailablePointCloudTranscoder;

impl PointCloudTranscoder for UnavailablePointCloudTranscoder {
    fn transcode(
        &self,
        _source: &Path,
        _destination: &Path,
        _operation_id: &str,
        _format: PointCloudExportFormat,
        _crs_wkt: Option<&str>,
        _cancellation: &CancellationToken,
        _progress: &mut dyn FnMut(u64, u64),
    ) -> Result<PointCloudTranscodeSummary, ProductExportError> {
        Err(ProductExportError::InvalidRequest(
            "point-cloud transcoder capability was not supplied".into(),
        ))
    }
}
...
pub fn export_product(
    ...
    export_product_with_transcoder(
        ...
        cancellation,
        progress,
        &UnavailablePointCloudTranscoder,
    )
}

pub fn export_product_with_transcoder(
    request: &ProductExportRequest,
    cancellation: &CancellationToken,
    mut progress: impl FnMut(u64, u64),
    pointcloud_transcoder: &dyn PointCloudTranscoder,
) -> Result<ProductExportSummary, ProductExportError> {
    ...
    if let ProductExportConversion::PointCloud { format, crs_wkt } = &request.source.conversion {
        ...
        let summary = pointcloud_transcoder.transcode(
            &source,
            &destination,
            ...
            *format,
            crs_wkt.as_deref(),
            cancellation,
            &mut |completed_points, total_points| {
