...
use crate::gcp_import::{import_gcp_csv_file_with_cancel, GcpCsvImportError, GcpCsvImportResult};
...
use crate::photolab_project::{JournalCommandState, PhotolabJournalEntry, PhotolabProjectManifest};
use himmelcad_model::entity::{
...
use himmelcad_transform::photolab_crs::{
    FrozenImportTransformation, HeightReference, ProjectReferenceFrame, VerticalOperationMode,
};
...
pub enum GcpObservationEdit {
    ...
    Block {
        coordinate: crate::photolab_gcp::ImageCoordinate,
        reason: String,
    },
...
pub struct EditGcpObservationParams {
    ...
    pub image_id: crate::photolab_matching::ImageId,
...
pub struct EditGcpObservationResult {
    pub operation_id: String,
    pub collection_sha256: ObjectHash,
    pub restored_state: Option<crate::photolab_gcp::GcpObservationState>,
    pub autosave_generation: u64,
    pub journal_sequence: u64,
}
    ...
    let (action_name, restored_state) = match &params.edit {
        GcpObservationEdit::Block { coordinate, reason } => {
            let blocked = GcpObservation {
                ...
                state: crate::photolab_gcp::GcpObservationState::Blocked {
        ...
        GcpObservationEdit::Unblock => {
            ...
            if !matches!(
                ...
                crate::photolab_gcp::GcpObservationState::Blocked { .. }
            ) {
...
fn restore_observation_before_block(
    ...
    point_id: &GcpPointId,
    image_id: crate::photolab_matching::ImageId,
    ...
    for observation in params.observations {
        ...
        if let Some(index) = current {
            let preserves_manual = params.preserve_manual
                && matches!(
                    collection.observations[index].state,
                    crate::photolab_gcp::GcpObservationState::Manual { .. }
                )
                && !matches!(
                    observation.state,
                    crate::photolab_gcp::GcpObservationState::Manual { .. }
...
mod tests {
    ...
    use crate::gcp_import::import_gcp_csv_file;
    use crate::photolab_gcp::{
    ...
    use crate::photolab_project::initial_photolab_manifest;
    use himmelcad_transform::photolab_crs::{
    ...
    fn manual(point: &str, image: u32, x: f64) -> GcpObservation {
        GcpObservation {
            point_id: crate::photolab_gcp::GcpPointId(point.into()),
    ...
    fn observation_upsert_supports_predicted_and_replacement() {
        ...
        let predicted = GcpObservation {
            point_id: crate::photolab_gcp::GcpPointId("A".into()),
            image_id: ImageId(1),
        ...
        let result = create_gcp_optimization_snapshot_transaction(
            ...
            CreateGcpOptimizationSnapshotParams {
                ...
                scope: GcpOptimizationScope {
                    ...
                    point_ids: vec![crate::photolab_gcp::GcpPointId("A".into())],
