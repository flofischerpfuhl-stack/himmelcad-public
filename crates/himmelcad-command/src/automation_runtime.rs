...
use himmelcad_document::canonical_project_store::CanonicalStoredObject;
...
/// Process-internal verified CAS source used by bounded automation leases.
#[derive(Debug)]
pub struct AutomationObjectSource {
    pub metadata: CanonicalStoredObject,
    pub source: File,
    pub source_entity: Option<EntityVersionRef>,
    pub typed_artifact: Option<himmelcad_model::typed_artifact::TypedArtifactDescriptor>,
    pub representation_slot: Option<String>,
    pub geometry_ref: Option<ObjectHash>,
}

/// Narrow path-free document capability used by the automation command runtime.
pub trait AutomationDocument {
    type Error: std::fmt::Display;

    fn automation_entities(&self) -> Result<(u64, Vec<CanonicalEntity>), Self::Error>;
    fn automation_validate_transaction(
        &self,
        transaction: &CanonicalCommandTransaction,
    ) -> Result<(), String>;
    fn automation_object_source(
        &self,
        object_hash: &ObjectHash,
    ) -> Result<AutomationObjectSource, Self::Error>;
}
...
mod tests {
    ...
    use himmelcad_core::app_protocol::{
        AppProtocolRequest, AppProtocolRequestEnvelope, AppProtocolResponse,
        AppProtocolResponseEnvelope, APP_PROTOCOL_SCHEMA_ID,
    };
    use himmelcad_document::canonical_document::CanonicalEntityMutation;
    use himmelcad_document::canonical_import::CanonicalJsonObject;
    use himmelcad_document::canonical_project_store::{
    ...
    use himmelcad_model::entity_model::{
        built_in_type, CanonicalEntity, EntityTypeId, GeometryResource,
    };
    ...
    #[derive(Default)]
    struct CanonicalAppRuntime {
        store: Option<CanonicalProjectStore>,
    }

    impl CanonicalAppRuntime {
        fn open(&mut self, root: &PathBuf) -> Result<(), CanonicalProjectStoreError> {
            let mut store = CanonicalProjectStore::open(root)?;
            if store.document().entities().next().is_none() {
                ...
                let mut entity = CanonicalEntity {
                    ...
                    name: "Automation test project".to_owned(),
                    owner: None,
                    layer_ids: Vec::new(),
                    placement: None,
                    ...
                    style_ref: None,
                    schema_version: 1,
                    version_hash: ObjectHash::of_bytes(b"pending automation root"),
                };
        ...
        fn automation_entities(
            &self,
        ) -> Result<(u64, Vec<CanonicalEntity>), CanonicalProjectStoreError> {
            let store = self.store.as_ref().expect("open test project");
            Ok((
        ...
        fn dispatch(&mut self, request: AppProtocolRequestEnvelope) -> AppProtocolResponseEnvelope {
            let response = match request.request {
                AppProtocolRequest::ExecuteCanonicalTransaction(transaction) => {
                    AppProtocolResponse::TransactionAccepted(
                        self.store
                            .as_mut()
                            .expect("open test project")
                            .commit_transaction(transaction)
                            .expect("commit test transaction"),
                    )
                }
                _ => unreachable!("automation runtime tests commit transactions only"),
            };
    ...
    impl AutomationDocument for CanonicalAppRuntime {
        type Error = CanonicalProjectStoreError;

        fn automation_entities(&self) -> Result<(u64, Vec<CanonicalEntity>), Self::Error> {
            CanonicalAppRuntime::automation_entities(self)
        }
        ...
        fn automation_object_source(
            ...
            let source_entity = store.document().entities().find_map(|entity| {
                (entity.components_ref == *object_hash
                    || entity.attributes_ref == *object_hash
                    || entity.relations_ref == *object_hash)
                    .then(|| EntityVersionRef::from_entity(entity))
            });
