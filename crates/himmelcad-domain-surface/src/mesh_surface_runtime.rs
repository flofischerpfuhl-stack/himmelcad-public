...
pub struct SurfacePublishResult {
    ...
    pub area: f64,
    pub z_range: [f64; 2],
    pub residual: crate::mesh_surface::SurfaceResidual,
...
/// Domain-neutral live document view consumed by surface commands.
#[derive(Debug, Clone)]
pub struct SurfaceDocumentSnapshot {
    pub entries: Vec<SurfaceAdmissionSnapshot>,
    pub draw_curves: Vec<SurfaceDrawCurveSnapshot>,
}

#[derive(Debug, Clone)]
pub struct SurfaceAdmissionSnapshot {
    pub admission: CanonicalRepresentationAdmission,
    pub dataset: Option<himmelcad_io::CanonicalPreparedDataset>,
}

/// Minimal immutable curve snapshot needed by surface source capture.
#[derive(Debug, Clone)]
pub struct SurfaceDrawCurveSnapshot {
    pub entity_id: String,
    pub revision: u64,
    pub name: String,
    pub closed: bool,
...
/// Exact document interface specialization accepted by this domain runtime.
pub trait SurfaceRuntimeDocument:
    SurfaceDocumentCommands<Snapshot = SurfaceDocumentSnapshot, StagedImport = CanonicalStagedImport>
{
}

impl<T> SurfaceRuntimeDocument for T where
    T: SurfaceDocumentCommands<
        Snapshot = SurfaceDocumentSnapshot,
        StagedImport = CanonicalStagedImport,
    >
{
}
...
pub fn create_surface_draft(
    runtime: &impl SurfaceRuntimeDocument,
    ...
    let bootstrap = runtime.surface_snapshot()?;
    let draw = &bootstrap.draw_curves;
        ...
        let name = if let Some(curve) = draw_curve {
            ...
            match &entry.admission.resolved_geometry {
                ...
                GeometryObject::PointCloud { .. } => {
                    ...
                    points.extend(runtime.surface_sample_point_cloud(
                        SurfacePointCloudSampleRequest {
                            source: source.source.clone(),
                            input_root: input,
                            source_id: source.source.id.0.clone(),
                            spacing: request.rules.thin_cloud_spacing,
                            placement: placement.0,
                            visible_classes: source.visible_classes.clone(),
                        },
                        cancellation,
                    )?);
                }
                GeometryObject::ElevationSurface { surface } => match surface.as_ref() {
                    ElevationSurfaceGeometry::Grid {
                        ...
                        let bytes = runtime
                            .surface_height_grid_bytes(&source.source.id)
                            .or_else(|_| runtime.surface_object_bytes(&raster.object_hash))?;
                        let mut object = Cursor::new(bytes);
...
pub fn publish_persisted_surface(
    runtime: &mut impl SurfaceRuntimeDocument,
    draft_id: &str,
    ...
    cancellation.check()?;
    progress(0.82, "Bake");
    let commit = runtime.surface_publish_staged_import(&staged, &command_id, &|| {
        cancellation.is_cancel_requested()
    })?;
    anyhow::ensure!(
        commit.admission_count > 0,
        "surface publish returned no admission"
    );
    let dataset = staged
        .package
        .datasets
...
fn preview_surface_edit(
    ...
    evaluate: impl FnOnce(
        ...
        &SurfaceEditRegion,
        &CancellationToken,
    ) -> Result<SurfaceEditResult, crate::mesh_surface::SurfaceEditError>,
) -> Result<SurfaceEditPreview> {
...
pub fn bake_surface_edit(
    ...
    let commit = runtime.surface_publish_staged_import(&staged, &request.command_id, &|| {
        cancellation.is_cancel_requested()
    })?;
    let dataset = staged
        .package
...
fn load_surface(
    ...
    Ok(LoadedSurface {
        ...
        mesh: SurfaceMesh {
            source_residual: crate::mesh_surface::SurfaceResidual {
                count: positions.len(),
                mean_absolute: 0.0,
...
fn read_object(runtime: &impl SurfaceRuntimeDocument, hash: &ObjectHash) -> Result<Vec<u8>> {
    runtime.surface_object_bytes(hash)
}
...
fn edit_root(runtime: &impl SurfaceRuntimeDocument, edit_id: &str) -> Result<PathBuf> {
    validate_id(edit_id, "editId")?;
    Ok(runtime
        .surface_project_root()?
...
fn draft_root(runtime: &impl SurfaceRuntimeDocument, draft_id: &str) -> Result<PathBuf> {
...
mod tests {
    ...
    fn mesh_surface_edit_checkpoint_restarts_without_partial_generation() {
        ...
        let checkpoint = SurfaceEditCheckpoint {
            ...
            region: SurfaceEditRegion {
                source: crate::mesh_surface::SurfaceRegionSource::Fence,
