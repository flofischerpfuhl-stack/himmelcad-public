...
pub use himmelcad_prepared::raster::{
    GdalAudit, OrthomosaicElevationSupport, OrthophotoSource, RasterBounds, RasterBuildSummary,
...
pub struct RasterRuntimeMemory {
    ...
    sink: Arc<dyn RasterMemorySink>,
    worker_plan_override: Option<WorkerMemoryLimitPlan>,
}

impl RasterRuntimeMemory {
    #[must_use]
    pub fn new<S>(plan: RasterPreparationStagePlan, sink: S) -> Self
    where
        S: RasterMemorySink + 'static,
    {
        Self {
            plan,
            sink: Arc::new(sink),
    ...
    #[doc(hidden)]
    pub fn with_worker_plan_override(mut self, plan: WorkerMemoryLimitPlan) -> Self {
...
impl RasterRuntime {
    ...
    pub async fn execute<P>(
        ...
        cancellation: &CancellationToken,
        checkpoint_sink: Option<&dyn RasterCheckpointSink>,
        memory: Option<RasterRuntimeMemory>,
        mut progress: P,
            ...
            tokio::task::spawn_blocking(move || {
                publish_prepared_elevation_hierarchy(
                    ...
                    PreparedElevationHierarchyOptions {
                        ...
                        diagonal:
                            himmelcad_model::entity_model::RasterCellDiagonal::TopLeftToBottomRight,
                    },
        ...
        P: FnMut(RasterProgress),
    {
        ...
        loop {
            ...
            if let Some(sink) = checkpoint_sink.filter(|sink| sink.accepts_raster_checkpoints()) {
                let sequence = u64::try_from(checkpoint.completed.len()).unwrap_or(u64::MAX);
                sink.record_raster_committed(
                    sequence,
                ...
                .await
                .map_err(RasterRuntimeError::CheckpointSink)?;
            }
        ...
        let (mut command, worker_limit_plan) =
            if let Some(override_plan) = memory.and_then(|memory| memory.worker_plan_override) {
                #[cfg(target_os = "linux")]
                {
                    ...
                    (
                        himmelcad_process::worker::worker_command_for_plan(
                            launcher,
                            &executable,
                            override_plan,
                        ),
            ...
            if status_indicates_memory_limit(&status, &stderr) {
                if let (Some(memory), Some(limit)) = (memory, worker_limit_plan) {
                    memory
                        ...
                        .map_err(RasterRuntimeError::MemoryRecord)?;
