...
function mockBridgeSource() {
  return `(() => {
    ...
    const call=async(method)=>{
      ...
      if(method==='photolab.jobs.list')return [{schemaVersion:1,id:'visual-depth',kind:'buildDepthMaps',origin:'job',configHash:hash,inputHash:hash,state:{kind:'running'},progress:{stage:{kind:'depthEstimation',index:1,stageCount:3,label:'Estimate depth'},metrics:{completedUnits:42,totalUnits:100,completedBytes:0}},createdAtUnixMs:0,startedAtUnixMs:0,memory:{envelopeBytes:30064771072,stages:[{stage:'Extract ALIKED',peakRssBytes:15660000000,workers:1,parameters:{maxImageEdge:8192}},{stage:'Match ALIKED with LightGlue',peakRssBytes:7180435456,workers:2,parameters:{keypointCap:24000,sequentialPairBatches:false}}],degradations:[{kind:'matchingKeypointsCapped',from:24000,to:18000}],observations:[]}},{schemaVersion:1,id:'visual-archive',kind:'archiveSave',origin:'sideOperation',configHash:hash,inputHash:hash,state:{kind:'running'},progress:{stage:{kind:'featureExtraction',index:0,stageCount:1,label:'Writing archive'},metrics:{completedUnits:0,completedBytes:0}},createdAtUnixMs:0,startedAtUnixMs:0}];
