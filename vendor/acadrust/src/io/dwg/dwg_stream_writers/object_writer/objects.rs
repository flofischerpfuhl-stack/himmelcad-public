...
impl<'a> DwgObjectWriter<'a> {
    ...
    fn write_plot_settings_data(&mut self, plot_flags: i16, layout: &crate::objects::Layout) {
        ...
        let (paper_width, paper_height) = if layout.paper_width > 0.0 && layout.paper_height > 0.0 {
        ...
        let num = if layout.plot_scale_numerator != 0.0 {
            layout.plot_scale_numerator
        } else {
            1.0
        };
        let den = if layout.plot_scale_denominator != 0.0 {
            layout.plot_scale_denominator
        } else {
            1.0
        };
        ...
        let factor = if layout.plot_scale_factor != 0.0 {
            layout.plot_scale_factor
        } else {
            1.0
        };
        ...
        if self.version.r2004_plus() {
            ...
            let dpi = if layout.shade_plot_dpi != 0 {
                layout.shade_plot_dpi
            } else {
                300
            };
    ...
    fn write_plot_settings_obj(&mut self, ps: &PlotSettings) {
        ...
        self.write_common_non_entity_data(type_code, ps.handle, ps.owner, &[], &None);
    ...
    fn write_xrecord(&mut self, xrec: &XRecord) {
        self.write_common_non_entity_data(common::OBJ_XRECORD, xrec.handle, xrec.owner, &[], &None);
