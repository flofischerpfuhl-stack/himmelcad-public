...
impl<'a> DwgObjectWriter<'a> {
    ...
    fn write_line_type(&mut self, ltype: &crate::tables::LineType) {
        ...
        for seg in &ltype.elements {
            ...
            self.writer
                .write_raw_double(c.map_or(0.0, |cx| cx.offset[0])); // offset x
            self.writer
                .write_raw_double(c.map_or(0.0, |cx| cx.offset[1])); // offset y
            self.writer.write_bit_double(c.map_or(1.0, |cx| cx.scale)); // scale
            self.writer
                .write_bit_double(c.map_or(0.0, |cx| cx.rotation)); // rotation
    ...
    fn write_view(&mut self, view: &crate::tables::View) {
        ...
        if self.version.r2007_plus() {
            ...
            self.writer.write_bit_double(0.0);
            self.writer
                .write_cm_color(&crate::types::Color::from_index(250));
        }
    ...
    fn write_vport_entries(&mut self) {
        let mut entries: Vec<_> = self.document.vports.iter().map(|v| v.clone()).collect();
        ...
        if let Some(ref ext) = self.model_space_extents {
            ...
            for vp in &mut entries {
                if vp.name == "*Active" {
                    ...
                    let cx = vp.view_target.x + cos_t * vp.view_center.x + sin_t * vp.view_center.y;
                    let cy = vp.view_target.y - sin_t * vp.view_center.x + cos_t * vp.view_center.y;
    ...
    fn write_appid_entries(&mut self) {
        let entries: Vec<_> = self.document.app_ids.iter().map(|a| a.clone()).collect();
        for app in &entries {
    ...
    fn write_dimstyle(&mut self, ds: &crate::tables::DimStyle) {
        ...
        if self.version.r13_14_only() {
            ...
            // DIMCLRD BS 176
            self.writer
                .write_cm_color(&crate::types::Color::from_index(ds.dimclrd));
            // DIMCLRE BS 177
            self.writer
                .write_cm_color(&crate::types::Color::from_index(ds.dimclre));
            // DIMCLRT BS 178
            self.writer
                .write_cm_color(&crate::types::Color::from_index(ds.dimclrt));
        }
        ...
        if self.version.r2007_plus() {
            ...
            // DIMTFILLCLR CMC 70
            self.writer
                .write_cm_color(&crate::types::Color::from_index(ds.dimtfillclr));
        }
                    ...
                    EntityType::PolyfaceMesh(e) => {
                        for v in &e.vertices {
                            if !v.common.handle.is_null() {
                                expanded.push(v.common.handle);
                            }
                        }
                        for f in &e.faces {
                            if !f.common.handle.is_null() {
                                expanded.push(f.common.handle);
                            }
                        }
                    ...
                    EntityType::Insert(e) if e.has_attributes() => {
                        for att in &e.attributes {
                            if !att.common.handle.is_null() {
                                expanded.push(att.common.handle);
                            }
                        }
                    }
                    _ => {}
        ...
        let base_pt = record
            ...
            .find_map(|eh| {
                if let Some(EntityType::Block(b)) = self
                    ...
                    .entity_index
                    .get(eh)
                    .map(|&idx| &self.document.entities[idx])
                {
                    Some(b.base_point)
                } else {
                    None
