...
impl<R: Read + Seek> DwgReader<R> {
        ...
        if !handle_map.is_empty() {
            if let Ok(objects_buf) = self.get_section_buffer("AcDb:AcDbObjects", &info) {
                match crate::io::dwg::dwg_stream_readers::object_reader::DwgObjectReader::new(
                    ...
                    Ok(obj_reader) => {
                        let mut builder =
                            crate::io::dwg::dwg_document_builder::DwgDocumentBuilder::new(
                                obj_reader,
                            );
    ...
    pub fn get_section_buffer(
        ...
        for page in &section.pages {
            ...
            if let Some(&(page_offset, _page_size)) =
                info.page_records.get(&(page.page_number as i32))
            {
                let page_result = if encoding == 1 {
                    ...
                    (|| -> Result<Vec<u8>, DxfError> {
                        self.stream
                            .seek(SeekFrom::Start(AC21_FILE_HEADER_SIZE + page_offset as u64))?;
    ...
    fn get_section_buffer_ac18(
        ...
        for page in &section.pages {
            ...
            if dst_start < total_size {
                ...
                result[dst_start..dst_start + copy_len].copy_from_slice(&decompressed[..copy_len]);
            }
        }
