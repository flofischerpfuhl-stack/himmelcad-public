...
use crate::objects::{
    BookColor, Dictionary, DictionaryVariable, DictionaryWithDefault, Group, ImageDefinition,
    ImageDefinitionReactor, Layout, MLineStyle, Material, MultiLeaderStyle, ObjectType,
    PlotSettings, RasterVariables, Scale, SortEntitiesTable, SpatialFilter, TableStyle,
    VisualStyle, WipeoutVariables, XRecord,
};
use crate::tables::*;
...
impl<'a, W: DxfStreamWriter> SectionWriter<'a, W> {
    ...
    pub fn write_header(&mut self, document: &CadDocument) -> Result<()> {
        ...
        self.write_header_variable("$CANNOSCALE", |w| {
            w.write_string(1, &hdr.current_annotation_scale)
        })?;
        self.write_header_variable("$CANNOSCALEVALUE", |w| {
            w.write_double(40, hdr.annotation_scale_value)
        })?;
    ...
    fn write_vport_entry(&mut self, vport: &VPort, owner: Handle) -> Result<()> {
        ...
        self.writer
            .write_i16(71, if vport.ucsfollow { 4 } else { 0 })?;
    ...
    fn write_dimstyle_entry(&mut self, dimstyle: &DimStyle, owner: Handle) -> Result<()> {
        ...
        if dimstyle.dimfxlon {
            self.writer.write_bool(290, true)?;
        }
        if dimstyle.dimtxtdirection {
    ...
    pub fn write_objects(&mut self, document: &CadDocument) -> Result<()> {
        ...
        for (handle, object) in document.objects.iter() {
            ...
            match object {
                ...
                ObjectType::GeoData(obj) => {
                    self.write_stub_handle_only("GEODATA", obj.handle, obj.owner)?
                }
    ...
    fn write_table_cell_style(
        &mut self,
        name: &str,
        style: &crate::objects::RowCellStyle,
    ) -> Result<()> {
    ...
    fn write_multileader(
        ...
        self.writer
            .write_i16(176, ctx.block_connection_type as i16)?;
        ...
        // Text left attachment type
        self.writer
            .write_i16(172, mleader.text_left_attachment as i16)?;
        ...
        // Text right attachment type
        self.writer
            .write_i16(173, mleader.text_right_attachment as i16)?;
        ...
        self.writer
            .write_i16(273, mleader.text_bottom_attachment as i16)?;

        // Text top attachment type
        self.writer
            .write_i16(272, mleader.text_top_attachment as i16)?;
    ...
    fn write_acad_table(&mut self, table: &table::Table, owner: Handle) -> Result<()> {
        ...
        if table.override_flag {
            override_flags |= 1;
        }
        if table.override_border_color {
            override_flags |= 2;
        }
        if table.override_border_line_weight {
            override_flags |= 4;
        }
        if table.override_border_visibility {
            override_flags |= 8;
        }
        ...
        // Break options
        self.writer
            .write_i32(94, table.break_options.bits() as i32)?;
        self.writer
            .write_i32(95, table.break_flow_direction as i32)?;
    ...
    fn write_table_cell(&mut self, cell: &TableCell) -> Result<()> {
        ...
        for content in &cell.contents {
            ...
            match content.value.value_type {
                ...
                CellValueType::Long => {
                    self.writer
                        .write_i32(90, content.value.numeric_value as i32)?;
                }
                _ => {}
            }
