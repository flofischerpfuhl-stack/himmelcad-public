...
impl<'a> SectionReader<'a> {
    ...
    pub fn read_header(&mut self, document: &mut CadDocument) -> Result<()> {
        ...
        while let Some(pair) = self.reader.read_pair()? {
            ...
            match var_name.as_str() {
                ...
                "$CAMERADISPLAY" => {
                    if let Some(p) = self.reader.read_pair()? {
                        hdr.camera_display = p.as_i16() == Some(1);
                    }
                }
                ...
                "$LWDISPLAY" => {
                    if let Some(p) = self.reader.read_pair()? {
                        hdr.lineweight_display = p.as_i16() == Some(1);
                    }
                }
                "$XEDIT" => {
                    if let Some(p) = self.reader.read_pair()? {
                        hdr.xedit = p.as_i16() == Some(1);
                    }
                }
                "$EXTNAMES" => {
                    if let Some(p) = self.reader.read_pair()? {
                        hdr.extended_names = p.as_i16() == Some(1);
                    }
                }
                "$PSTYLEMODE" => {
                    if let Some(p) = self.reader.read_pair()? {
                        hdr.plotstyle_mode = p.as_i16() == Some(1);
                    }
                }
                "$OLESTARTUP" => {
                    if let Some(p) = self.reader.read_pair()? {
                        hdr.ole_startup = p.as_i16() == Some(1);
                    }
                }
    ...
    fn read_visualstyle(&mut self) -> Result<Option<VisualStyle>> {
        ...
        while let Some(pair) = self.reader.read_pair()? {
            ...
            match pair.code {
                ...
                71 => {
                    if let Some(v) = pair.as_i16() {
                        obj.face_lighting_model = v;
                    }
                }
                72 => {
                    if let Some(v) = pair.as_i16() {
                        obj.face_lighting_quality = v;
                    }
                }
                73 => {
                    if let Some(v) = pair.as_i16() {
                        obj.face_color_mode = v;
                    }
                }
                90 => {
    ...
    fn read_multileader(&mut self) -> Result<Option<MultiLeader>> {
        ...
        while let Some(pair) = self.reader.read_pair()? {
            ...
            match pair.code {
                ...
                91 => {
                    if let Some(v) = pair.as_i32() {
                        ml.line_color = Color::from_index(v as i16);
                    }
                }
                40 => {
                    if let Some(v) = pair.as_double() {
                ...
                174 => {
                    if let Some(v) = pair.as_i16() {
                        ml.text_left_attachment =
                            crate::entities::multileader::TextAttachmentType::from(v);
                    }
                }
                175 => {
                    if let Some(v) = pair.as_i16() {
                        ml.text_right_attachment =
                            crate::entities::multileader::TextAttachmentType::from(v);
                    }
                }
                176 => {
                    if let Some(v) = pair.as_i16() {
                        ml.text_angle_type = crate::entities::multileader::TextAngleType::from(v);
                    }
                }
                291 => {
                    if let Some(v) = pair.as_bool() {
    ...
    fn read_mesh(&mut self) -> Result<Option<Mesh>> {
        ...
        for (i, crease) in crease_values.into_iter().enumerate() {
            if i < mesh.edges.len() {
                mesh.edges[i].crease = Some(crease);
            }
        }
        ...
        while let Some(pair) = self.reader.read_pair()? {
            ...
            match pair.code {
                ...
                1 => {
                    if let Some(c) = cur.as_mut() {
                        ensure_content(c);
                        c.contents.last_mut().unwrap().value = CellValue::text(&pair.value_string);
                    }
                }
                140 => {
                    if let (Some(c), Some(v)) = (cur.as_mut(), pair.as_double()) {
                ...
                _ => {
                    ...
                    let value = match XRecordValueType::from_code(pair.code) {
                        ...
                        XRecordValueType::Byte => pair
                            .as_i16()
                            .map(|v| XRecordValue::Byte(v as u8))
                            .unwrap_or(XRecordValue::String(pair.value_string.clone())),
                        XRecordValueType::Bool => pair
                            .as_i16()
                            .map(|v| XRecordValue::Bool(v != 0))
                            .unwrap_or(XRecordValue::String(pair.value_string.clone())),
