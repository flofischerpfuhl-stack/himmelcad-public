...
def prism(c):
    ...
    return [
        ...
        poly(c["w2"], [X, (X[0], X[1] + k), (Q[0] - 2, Q[1] + k), Q]),
    ]
...
AZ_LIGHT = (AZ_RAMP, AZ["m1"])
GR_LIGHT = (GR_RAMP, GR["g5"])

LOGOS = [
    # (file, title, group id, artwork, (ramp, rim colour))
    ("himmelcad", "Himmel:CAD", "Cloud-Low-Poly", cloud(), AZ_LIGHT),
    ("himmelcad-builder", "Himmel:CAD Builder – Hard Hat", "Hard-Hat-Low-Poly", flat.hard_hat_front(AZ), AZ_LIGHT),
    ("himmelcad-photolab-a-prism", "Himmel:CAD PhotoLab – Prism", "Prism-Low-Poly", prism(AZ), AZ_LIGHT),
    ("himmelcad-photolab-b-crystal", "Himmel:CAD PhotoLab – Crystal", "Crystal-Low-Poly", prism_crystal(AZ, None), AZ_LIGHT),
    ("himmelcad-cap", "Himmel:CAD Cap – Aperture", "Aperture-Low-Poly", aperture(AZ), AZ_LIGHT),
    ("himmelcad-weltview", "Himmel:CAD WeltView – Globe", "Globe-Low-Poly", flat.globe(AZ, lon0=-10, lat0=30), AZ_LIGHT),
    ("bergwork", "berg:work – Schlägel und Eisen", "Hammer-Pick-Low-Poly", flat.hammer_and_pick(GR), GR_LIGHT),
    ("bergwork-pdf", "berg:work PDF – Stone Tablet", "Tablet-Low-Poly", pdf_slab(GR, GR["red"]), GR_LIGHT),
    ("bergwork-image", "berg:work Image – Stone Frame", "Frame-Low-Poly", image_slab(GR, GR["red"]), GR_LIGHT),
]

for name, title, gid, art, (ramp, rim_col) in LOGOS:
    write(f"{name}.svg", svg(title, gid, art))
    write(f"{name}-on-light-rim.svg", svg(f"{title} (on light, rim)", gid, rim.rim(art, rim_col)))
    write(f"{name}-on-light-tone.svg", svg(f"{title} (on light, toned)", gid, rim.tone(art, ramp)))
