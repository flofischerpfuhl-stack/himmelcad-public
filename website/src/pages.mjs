...
function intro(kicker, title, text, extra = '') {
  return `<section class="page-intro"><p class="eyebrow">${kicker}</p><h1 class="display-title">${title}</h1><p class="lede">${text}</p>${extra}</section>`;
}

function productsGrid() {
  return `<div class="card-grid product-grid">
    <article class="card card--blue">${status('First release in progress', 'active')}<h3 class="display-title">PhotoLab</h3><p>Turns image and control data into aligned cameras, point clouds, elevation products, orthomosaics, meshes and splats.</p><a class="text-link" href="/photolab/">PhotoLab details →</a></article>
    <article class="card">${status('Internal build', 'works')}<h3 class="display-title">Builder</h3><p>The flagship: 3D-first Civil CAD with first-class 2D and 2.5D construction.</p><a class="text-link" href="/builder/">Builder details →</a></article>
    <article class="card">${status('MVP; not released', 'active')}<h3 class="display-title">Cap</h3><p>Mobile field capture that packages images, poses and quality evidence into a <code>.hcap</code> session.</p><a class="text-link" href="/cap-weltview/">Cap status →</a></article>
    <article class="card">${status('Publication planned', 'planned')}<h3 class="display-title">WeltView</h3><p>A read-only browser viewer for shared Himmel:CAD projects.</p><a class="text-link" href="/cap-weltview/#weltview">WeltView status →</a></article>
  </div>`;
}
...
function home() {
  return {
    ...
    <section class="section"><div class="section-heading"><p class="eyebrow">Four connected products</p><h2 class="display-title">One family.</h2><p>Shared data, rendering and commands where each product allows it. Every status below shows the current development state; none is a public release.</p></div>${productsGrid()}</section>
...
function photolab(ctx) {
  ...
  return {
    ...
    content: `${intro('PhotoLab · first release in progress', 'PhotoLab.', 'Images become measured spatial products offline. PhotoLab keeps the source data, coordinate choices, accuracy evidence and a record of how each result was made.', status('No public build', 'active'))}
...
function builder(ctx) {
  return {
    ...
    content: `${intro('Builder · flagship in development', 'Builder.', 'A 3D-first Civil CAD with first-class 2D and 2.5D construction. Builder keeps point clouds, terrain, CAD, raster, mesh and BIM work in one project.', status('No public build', 'active'))}
...
function capWeltview() {
  return {
    ...
    <section class="section split"><article>${status('Implemented MVP', 'works')}<h2>Himmel:CAD Cap</h2><p>A Flutter application for Android and iOS. Cap records phone imagery, poses, observations and quality evidence, then prepares a versioned, checksummed <code>.hcap</code> package for PhotoLab.</p><p>Capture data stays local unless the operator explicitly shares or uploads it. Reconstruction, CRS decisions and final accuracy reporting belong to PhotoLab.</p><div class="callout"><strong>Not released:</strong> supported devices, capture reliability, field accuracy, package recovery and complete Cap-to-PhotoLab results still need field evidence.</div></article><article id="weltview">${status('Browser viewer in progress', 'active')}<h2>Himmel:CAD WeltView</h2><p>A read-only browser viewer for shared projects. It uses the shared viewer and interface but cannot edit project data.</p><p>Publication remains a roadmap stage. The delivery method for large projects and full real-project loading still require validation.</p><div class="callout"><strong>Not released:</strong> there is no public WeltView service or project link.</div></article></section>`,
  };
}
...
function download(releases) {
  ...
  const cards = products
    .map(([key, name, copy]) => {
      ...
      if (!list.length)
        return `<article class="download-card">${status('No public build yet', 'active')}<h2>${name}</h2><p>${copy}</p><a class="text-link" href="${notify}">Ask for a release notification →</a></article>`;
